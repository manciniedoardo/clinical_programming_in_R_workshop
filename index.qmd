...
## Exercise 1b — High Systolic Blood Pressus Flag (`HISOBPFL`)
...
- This exercise will use the `ADS`L dataset you created in Exercise 1. You can also download it from the GitHub repo if you haven't completed the exercise.
:::
:::
...
::: fragment 
`ADVS` contains **one row per patient per visit per vital sign measurement** - so each patient has multiple rows the the dataset 
...
| `PARAM  `            | Long name for the test     | `"Systolic blood pressue (mmHg)` |
...
Which patient has the highest MAP and at what visit?
:::
...
# Highest MAP is 158.0 mmHg for 01-718-1355 at Week 24 visit (timepoint: AFTER STANDING FOR 1 MINUTE)
```
:::
...
::: fragment
:::{.callout-note title="ADSL"}
- This exercise will use the `ADVS` dataset you created in Exercise 1. You can also download it from the GitHub repo if you haven't completed the exercise.
:::
:::

::: fragment
**Task:** Create two plots of mean `"MAP"`/`"MAPV2"` over time (use visit number `AVISITN`), split by treatment arm ,(`TRT01A"`) and restricted to patients aged **\>65**. Optional: facet each plot into two, splitting by patients who have high systolic blood pressure (`HISOBPFL == "Y"`) and those who do not (`HISOBPFL == "N"`).
