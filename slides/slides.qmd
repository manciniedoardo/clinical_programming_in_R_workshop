...
::: fragment
Common R packages: `{ggplot2}`, `{gtsummary}`, `{rtables}`, `{Tplyr}`
:::
...
collected** — the numeric result is blank. Always exclude these records
before computing derived parameters like MAP, otherwise the calculation
will produce `NA`.
:::
...
::: callout-warning
`filter = VSSTAT != "NOT DONE" | is.na(VSSTAT)` removes records where
the vital sign was **not collected**. Without this, "NOT DONE" rows
(which have no numeric result) would be passed to the MAP formula and
produce `NA`. Most normal records have `VSSTAT` as `NA` — the `| is.na(VSSTAT)` keeps them.
:::
